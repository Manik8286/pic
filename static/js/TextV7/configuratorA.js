...
var Configurator = function() {
  ...
  StarmapPreview.prototype.buildPreview = function() {
    ...
    this.title = document.getElementById('itemConfiguratorForm:title').value
      .trim();
    // this.subtitle3 = document.getElementById('itemConfiguratorForm:subtitle3').value.trim();
    // this.subtitle4 = document.getElementById('itemConfiguratorForm:subtitle4').value.trim();
    ...
    if (this.datetimeInput == "" && this.datetime != "") {
      ...
      var dateTimeHoursSet = time.split(':')[0];
      var dateTimeMinutesSet = time.split(':')[1];
      // sets date and time into fields on starmap edit
      document.getElementById('itemConfiguratorForm:datetimeInput').value =
        date;
      ...
      this.datetime = this.datetimeInput + "T" + dateTimeHours + ":" +
        dateTimeMinutes + ":00%2B" + timezone // let's set midday by default
    } else {
      //Set default date
      var today = new Date().toISOString();
      var dt = today.split("T");
      var date = dt[0];
      ...
      document.getElementById('itemConfiguratorForm:datetimeInput').value =
        this.datetimeInput;
      document.getElementById('itemConfiguratorForm:datetime').value = this
        .datetime;
    }
    ...
    // Starmap final url
    var starmapSrc = this.serviceUrl + '/api/v1/png?w=' + this.formatWidth +
      "&h=" + this.formatHeight + "&pxw=" + fwSize + "&lat=" + this.latitude +
    ...
    // + "&t3=" + encodeURIComponent(this.subtitle3)
    // + "&t4=" + encodeURIComponent(this.subtitle4);
    ...
    starmapImg.onload = function() {
      starmapPreview = document.getElementById('canvas-preview');
  ...
  Preview.prototype.setupCanvas = function() {
    if (this.material == materials.blanket || this.material == materials.premiumBlanketPlus ||
      this.material ==
      materials.wallTap) {
      this.border = 30;
    }
  ...
  TowelPreview.prototype.setupImage = function(ox, oy) {
    new fabric.Image.fromURL(this.towelMapImage, function(img) {
      img.left = numberZero;
      img.top = numberZero;
      img.width = fbCanvas.width + 1;
      img.height = fbCanvas.height + 1;
  ...
  PaspartuPreview.prototype.setupBorder = function() {
    ...
    makeBorderPart(0, 0, passLeft, this.canvasHeight, "#FFFCF6", fbCanvas,
      false, false, "BorderLeft");
    makeBorderPart(0, 0, this.canvasWidth, passTop, "#FFFCF6", fbCanvas,
      false, false, "BorderTop");
    makeBorderPart((this.canvasWidth - passLeft), 0, passLeft, this.canvasHeight,
      "#FFFCF6", fbCanvas, false, false, "BorderRight");
    makeBorderPart(0, (this.canvasHeight - passTop), this.canvasWidth,
      passTop, "#FFFCF6", fbCanvas, false, false, "BorderBottom");
  ...
  function makeFrameBorderOver(canvaW, canvaH, border, canvas, color, cutout) {
    ...
    makeBorderPart(0, 0, canvaW, border, "transparent", canvas, false, false,
      "BorderTop"); // TOP
    makeBorderPart(0, canvaH - border, canvaW, border, "transparent", canvas,
      false, false, "BorderBottom"); // BOTTOM
    makeBorderPart(0, 0, border, canvaH, "transparent", canvas, false, false,
      "BorderLeft"); // LEFT
    makeBorderPart(canvaW - border, 0, border, canvaH, "transparent", canvas,
