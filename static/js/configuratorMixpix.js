...
var Configurator = function() {
  ...
  var formatChange = false, formatChangeF = false;
  var calcParams = true;
  var linkImageWidth = 0, linkImageHeight = 0;
  var linkImage = null, bgImage = null;
  var offX = parseFloat(-2), offY = parseFloat(-2), offXf = parseFloat(-2), offYf = parseFloat(-2), zoomVal = numberOne;
  var defaultPreview = 350, defaultHeight = 350;
  var RetinaDisplay = (window.devicePixelRatio > 1);
  var c0 = null, c1 = null, c2 = null;
  ...
  var colorPrevious = null, savedRotation = numberZero;
  var scaleMode = false, rotateAction = false; FrameLoaded = false;
  var rotationState = {};
  ...
  this.buildPreview = function() {
    ...
    if (color != undefined && color != null && (color.toLowerCase().indexOf('rgb') > -1) && (color.toLowerCase().indexOf('rgba') < 0)) {
      voidColor = color;
    }
    calcParams = true;
    if (!!document.getElementById('rotateMotif')) {
      savedRotation = parseInt(document.getElementById('itemConfiguratorForm:cutoutRotation').value);
    }
    if (preview != null) {
    ...
    if (typeof multiimage !== 'undefined' && multiimage == true) {
      ...
      } else if (material == materials.canvas || material == materials.rolledCanvas || material == materials.canvasPlus) {
        if ('FOLDED' == beffect) {
    ...
    if ($(".canvas-container").length && $("#thepreviewBox").length) {
      ...
      if (myCanvas.outerWidth() > myCanvasSpace.outerWidth()) {
        ...
        myCanvas.width(myCanvas.outerWidth() * scale).height(myCanvas.outerHeight() * scale);
  ...
  this.doScale = function() {
    scaleMode = true;
    zoomVal = numberOne + parseInt(document.getElementById("scaleCanvas").value) / 100;
	if (bgImage) {
  ...
  this.rotatePreview = function() {
    ...
    preview.sizeFactor = [ preview.sizeFactorR, preview.sizeFactorR = preview.sizeFactor ][0];
    linkImageWidth = linkImage.width * preview.sizeFactor;
    linkImageHeight = linkImage.height * preview.sizeFactor;
    calcParams = true;
    preview.imgHeight = [ preview.imgWidth, preview.imgWidth = preview.imgHeight ][0];
  ...
  this.validateParams = function() {
    ...
    var w = document.getElementById('itemConfiguratorForm:cutoutWidth').value.trim();
    var h = document.getElementById('itemConfiguratorForm:cutoutHeight').value.trim();

    if (isNaN(x) || isNaN(y) || isNaN(w) || isNaN(h) || x == "" || y == "" || w == "" || h == "") {
	  if (preview) {
  ...
  var Preview = function(item) {
    ...
    if (this.finish1 > numberZero) {
      var colorFinishing = tcolors[this.finish1] != null ? tcolors[this.finish1] : tcolors[this.finish2];
  ...
  function MultiImagePreview() { // multiimage
    ...
    this.rotation = (item.data('rotation') == "") ? 0 : parseInt(item.data('rotation'));
    this.productid = item.data('id');
    this.format = [ parseFloat(item.data('width')), parseFloat(item.data('height')) ];
    this.fotoBox = bilderBox;
    this.fotoBoxRetro = bilderBoxRetro;
    this.trim = (typeof item.data('trim') === 'undefined' || item.data('trim') == "") ? 0 : parseFloat(item.data('trim'));
  ...
  MultiImagePreview.prototype.buildPreview = function() {
    ...
    document.getElementById('itemConfiguratorForm:cutoutX').value = parseFloat(cropInfo.x);
    document.getElementById('itemConfiguratorForm:cutoutY').value = parseFloat(cropInfo.y);
    document.getElementById('itemConfiguratorForm:cutoutWidth').value = parseFloat(cropInfo.width);
    document.getElementById('itemConfiguratorForm:cutoutHeight').value = parseFloat(cropInfo.height);
    Preview.prototype.buildPreview.call(this);
  };
  ...
  Preview.prototype.buildCanvas = function() {
    preview.setupCanvas();
    if (this.frame >= dekoFramesFix && this.material != materials.pass && this.material != materials.canvas) {
    ...
    var imgSize = 0;
    if ((hratio > wratio && this.formatWidth == this.formatHeight) || this.formatHeight > this.formatWidth) {
      imgSize = newdim[0];
    } else {
      imgSize = newdim[1];
    }
    ...
    if (linkImage == null || this.colorchange) {
      ...
      this.img.onload = function() {
        linkImage = preview.img;
        if ((hratio > wratio && preview.formatWidth == preview.formatHeight) || preview.formatHeight > preview.formatWidth) {
      ...
      preview.img.src = "dynamicimage/" + this.imageid + "?size=" + (getSize | 0) + "&effect=" + this.coloreffect;
    } else {
      if ((hratio > wratio && this.formatWidth == this.formatHeight) || this.formatHeight > this.formatWidth) {
        linkImageHeight = imgSize;
        linkImageWidth = linkImageHeight * linkImage.width / linkImage.height;
  ...
  Preview.prototype.setupRotated = function(wratio, hratio) {
    ...
    ctx.translate(c0.width / 2, c0.height / 2);
    ctx.rotate(270 * Math.PI / 180);
    ctx.drawImage(preview.img, -preview.img.width * 0.5, -preview.img.height * 0.5);
    ctx.restore();
    ...
    if ((hratioR > wratioR && this.formatWidth == this.formatHeight) || this.formatHeight > this.formatWidth) {
      c1.width = newdim[1];
  ...
  Preview.prototype.setupCanvas = function() {
    if (this.material == materials.blanket || this.material == materials.premiumBlanket || this.material == materials.premiumBlanketPlus || this.material == materials.wallTap) {
      this.border = 30;
    }
    if (this.formatWidth > this.formatHeight) {
  ...
  console.log(this.originalHeight);
    ...
    if (this.originalWidth == 142 && this.originalHeight == 142) {
      this.knownConstant = 202;
    } else if (this.originalWidth == 244 && this.originalHeight == 143 || this.originalWidth == 143 && this.originalHeight == 244) {
      this.knownConstant = 247;
    } else if (this.originalWidth == 300 && this.originalHeight == 220 || this.originalWidth == 220 && this.originalHeight == 300) {
      this.knownConstant = 250;
    } else if (this.originalWidth == 324 && this.originalHeight == 222 || this.originalWidth == 222 && this.originalHeight == 324) {
      this.knownConstant = 254;
    } else if (this.originalWidth == 500 && this.originalHeight == 300 || this.originalWidth == 300 && this.originalHeight == 500) {
      this.knownConstant = 274;
    } else if (this.originalWidth == 320 && this.originalHeight == 320) {
      this.knownConstant = 257;
    } else if (this.originalWidth == 406 && this.originalHeight == 304 || this.originalWidth == 304 && this.originalHeight == 406) {
      this.knownConstant = 284;
    } else if (this.originalWidth == 324 && this.originalHeight == 324) {
      this.knownConstant = 254;
    } else if (this.originalWidth == 660 && this.originalHeight == 406 || this.originalWidth == 406 && this.originalHeight == 660) {
      this.knownConstant = 289;
    }
    ...
    if (this.originalWidth == 142 && this.originalHeight == 142 || this.originalWidth == 320 && this.originalHeight == 320 || this.originalWidth == 324 && this.originalHeight == 324) {
      ...
      this.canvasWidth = 350;
      this.canvasHeight = 238;
    }

    this.frameSrc = "/staticimages/configurator/passepartout/frame_" + this.originalWidth + "x" + this.originalHeight + "mm_" + decoFrames[('f' + this.frame)] + ".png";
    if (this.originalWidth < this.originalHeight) {
      this.canvasHeight = [ this.canvasWidth, this.canvasWidth = this.canvasHeight ][0];
    }
  ...
  StandardCushionPreview.prototype.setupCanvas = function() {
	...
	this.frameSrc = "/assets/img/checkout/cushion/standard-cushion-mask.png";
  ...
  PremiumCushionPreview.prototype.setupCanvas = function() {
    ...
    if (this.formatWidth > this.formatHeight) {
      ...
      this.printHeight = this.formatHeight / this.formatWidth * this.printWidth;
      this.phoneMapImage = "/assets/img/checkout/cushion/premium-cushion-mask.png";
    } else {
  ...
  MousePadPreview.prototype.setupCanvas = function() {
	...
	if (this.formatWidth > this.formatHeight) {
	  ...
	  this.phoneMapImage = "/assets/img/checkout/mousepad/mousepad-mask-landscape.png";
    } else {
	  this.canvasWidth = 291;
	  this.canvasHeight = 350;
	  this.phoneMapImage = "/assets/img/checkout/mousepad/mousepad-mask-portrait.png";
    }
  ...
  PhoneCasePreview.prototype.setupCanvas = function() {
    ...
    // Samsung S4
    if (this.formatWidth == 138.8 && this.formatHeight == 71.8 || this.formatWidth == 132 && this.formatHeight == 68) {
      this.canvasWidth = 350;
      this.canvasHeight = 204;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s4_topH.png";
    } else if (this.formatWidth == 71.8 && this.formatHeight == 138.8 || this.formatWidth == 68 && this.formatHeight == 132) {
      this.canvasWidth = 204;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s4_top.png";

    // Samsung S5
    } else if (this.formatWidth == 144.2 && this.formatHeight == 74.4 || this.formatWidth == 144 && this.formatHeight == 74) {
      this.canvasWidth = 350;
      this.canvasHeight = 202;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s5_topH.png";
    } else if (this.formatWidth == 74.4 && this.formatHeight == 144.2 || this.formatWidth == 74 && this.formatHeight == 144) {
      this.canvasWidth = 202;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s5_top.png";

    // Samsung S6
    } else if (this.formatWidth == 143 && this.formatHeight == 70.1 || this.formatWidth == 141 && this.formatHeight == 68.1) {
      this.canvasWidth = 350;
      this.canvasHeight = 193;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s6_topH.png";
    } else if (this.formatWidth == 70.1 && this.formatHeight == 143 || this.formatWidth == 68.1 && this.formatHeight == 141) {
      this.canvasWidth = 193;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s6_top.png";

    // Samsung S6 Edge
    } else if (this.formatWidth == 142.1 && this.formatHeight == 70.1 || this.formatWidth == 140.1 && this.formatHeight == 68.1) {
      this.canvasWidth = 350;
      this.canvasHeight = 195;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s6e_topH.png";
    } else if (this.formatWidth == 70.1 && this.formatHeight == 142.1 || this.formatWidth == 68.1 && this.formatHeight == 140.1) {
      this.canvasWidth = 195;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s6e_top.png";

    // Samsung S7
    } else if (this.formatWidth == 144.2 && this.formatHeight == 71.4 || this.formatWidth == 140.5 && this.formatHeight == 68) {
      this.canvasWidth = 350;
      this.canvasHeight = 194;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s7_topH.png";
    } else if (this.formatWidth == 71.4 && this.formatHeight == 144.2 || this.formatWidth == 68 && this.formatHeight == 140.5) {
      this.canvasWidth = 194;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s7_top.png";

    // Samsung S7 Edge
    } else if (this.formatWidth == 152.3 && this.formatHeight == 74 || this.formatWidth == 149 && this.formatHeight == 70.45) {
      this.canvasWidth = 350;
      this.canvasHeight = 189;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s7e_topH.png";
    } else if (this.formatWidth == 74 && this.formatHeight == 152.3 || this.formatWidth == 70.45 && this.formatHeight == 149) {
      this.canvasWidth = 189;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s7e_top.png";

    // Samsung S8
    } else if (this.formatWidth == 150.9 && this.formatHeight == 70.1) {
      this.canvasWidth = 350;
      this.canvasHeight = 200;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s8_topH.png";
    } else if (this.formatWidth == 70.1 && this.formatHeight == 150.9) {
      this.canvasWidth = 200;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/samsung_s8_top.png";

    // iPhone 4
    } else if (this.formatWidth == 119.8 && this.formatHeight == 63.2 || this.formatWidth == 116 && this.formatHeight == 59 ) {
      this.canvasWidth = 350;
      this.canvasHeight = 201;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone4_topH.png";
    } else if (this.formatWidth == 63.2 && this.formatHeight == 119.8 || this.formatWidth == 59 && this.formatHeight == 116) {
      this.canvasWidth = 201;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone4_top.png";

    // iPhone 5
    } else if (this.formatWidth == 125 && this.formatHeight == 59.8 || this.formatWidth == 125 && this.formatHeight == 59) {
      this.canvasWidth = 350;
      this.canvasHeight = 193;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone5_topH.png";
    } else if (this.formatWidth == 59.8 && this.formatHeight == 125 || this.formatWidth == 59 && this.formatHeight == 125) {
      this.canvasWidth = 193;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone5_top.png";

    // iPhone 6
    } else if (this.formatWidth == 137.9 && this.formatHeight == 68.8 || this.formatWidth == 136 && this.formatHeight == 65) {
      this.canvasWidth = 350;
      this.canvasHeight = 193;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone6_topH.png";
    } else if (this.formatWidth == 68.8 && this.formatHeight == 137.9 || this.formatWidth == 65 && this.formatHeight == 136) {
      this.canvasWidth = 193;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone6_top.png";

    // iPhone 6 Plus
    } else if (this.formatWidth == 155.3 && this.formatHeight == 78 || this.formatWidth == 155.8 && this.formatHeight == 79.6) {
      this.canvasWidth = 350;
      this.canvasHeight = 200;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone6p_topH.png";
    } else if (this.formatWidth == 78 && this.formatHeight == 155.3 || this.formatWidth == 79.6 && this.formatHeight == 155.8 ) {
      this.canvasWidth = 200;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone6p_top.png";

    // iPhone 7
    } else if (this.formatWidth == 138.5 && this.formatHeight == 69.8 || this.formatWidth == 136.01 && this.formatHeight == 65) {
      this.canvasWidth = 350;
      this.canvasHeight = 193;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone7_topH.png";
    } else if (this.formatWidth == 69.8 && this.formatHeight == 138.5 || this.formatWidth == 65 && this.formatHeight == 136.01) {
      this.canvasWidth = 193;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone7_top.png";

    // iPhone 7 Plus
    } else if (this.formatWidth == 155.3 && this.formatHeight == 78.5 || this.formatWidth == 156 && this.formatHeight == 80) {
      this.canvasWidth = 350;
      this.canvasHeight = 201;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone7p_topH.png";
    } else if (this.formatWidth == 78.5 && this.formatHeight == 155.3 || this.formatWidth == 80 && this.formatHeight == 156) {
      this.canvasWidth = 201;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone7p_top.png";

    // iPhone 8
    } else if (this.formatWidth == 139 && this.formatHeight == 69.9) {
      this.canvasWidth = 350;
      this.canvasHeight = 200;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone8_topH.png";
    } else if (this.formatWidth == 69.9 && this.formatHeight == 139) {
      this.canvasWidth = 200;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone8_top.png";

    // iPhone 8 Plus
    } else if (this.formatWidth == 155.6 && this.formatHeight == 78.7) {
      this.canvasWidth = 350;
      this.canvasHeight = 200;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone8p_topH.png";
    } else if (this.formatWidth == 78.7 && this.formatHeight == 155.6) {
      this.canvasWidth = 200;
      this.canvasHeight = 350;
      this.phoneMapImage = "/staticimages/configurator/handycase/iphone8p_top.png";
    }
    this.printWidth = this.canvasWidth;
    this.printHeight = this.canvasHeight;
    this.lineWidth = numberZero;
  ...
  TshirtPreview.prototype.setupCanvas = function() {
    ...
    if (this.formatHeight == 300 || this.formatWidth == 300) { // Man tshirt
      ...
      this.leftBorder = 118;
      this.topBorder = 73;
      this.tshirtMap = "/staticimages/configurator/shirts/tshirt_male_" + this.tshirtType + this.colorname + ".png";
    } else { // Woman tshirt
      this.printWidth = 92; // 20 cm
      this.printHeight = 115; // 25 cm
      this.leftBorder = 129;
      this.topBorder = 84;
      this.tshirtMap = "/staticimages/configurator/shirts/tshirt_female_" + this.tshirtType + this.colorname + ".png";
    }
  ...
  MultiImagePreview.prototype.setupCanvas = function() {
    if (this.productid == this.fotoBox) {
      ...
      if( $(window).width() < 400 ) {
       this.canvasWidth = $(window).width() - 55;
      } else {
       this.canvasWidth = 350;
      }
  ...
  MultiImagePreview.prototype.setCanvas = function() {
    ...
    fbCanvas.on("object:modified", function(e) {
      ...
      if (typeof previewCropThumbnail === "undefined") {
          return;
      } else {
        preview.cropPreview();
      }
  ...
  Preview.prototype.completePreview = function(doSave) {
    var offsetX = (linkImageWidth - this.printWidth) / 2;
    var offsetY = (linkImageHeight - this.printHeight) / 2;
    ...
    if (this.lineWidth > numberZero) {
      if (this.material == materials.pillow) {
        ...
        $('#ch').css('top', (this.border + this.border10mm) + 'px').css('bottom', (this.border + this.border10mm) + 'px');
        $('#cw').css('left', (this.border + this.border10mm) + 'px').css('right', (this.border + this.border10mm) + 'px');
      }
    }
  ...
  MultiImagePreview.prototype.setupOffset = function(offsetX, offsetY) {
    var cropInfoX = document.getElementById('itemConfiguratorForm:cutoutX').value;
  ...
  CanvasPreview.prototype.setupImage = function(ox, oy) {
    Preview.prototype.setupImage.call(this, offX - this.border, offY - this.border);
  }

  PaspartuPreview.prototype.setupImage = function(ox, oy) {
    Preview.prototype.setupImage.call(this, offX - this.leftBorder, offY - this.topBorder);
  }
  ...
  Preview.prototype.setupBorder = function() {
    if (this.frame >= dekoFramesFix) {
      ...
      if (this.border > numberZero) {
        makeColorBorder(fbCanvas.width, fbCanvas.height, this.border, voidColor, false);
      }
	...
	insertSafeZone(this.material, this.formatWidth, this.formatHeight, this.canvasWidth, this.canvasHeight, this.border, this.border);
  }
  ...
  CanvasPreview.prototype.setupBorder = function() {
    if (this.frame > 0) {
	  makeFrameBorder(fbCanvas.width, fbCanvas.height, this.border, "rgba(255, 255, 255, 0.5)", false);
	}
	else {
	  makeColorBorder(fbCanvas.width, fbCanvas.height, this.border, "rgba(255, 255, 255, 0.5)", false);
	  preview.drawBorders();
    }
  ...
  CanvasPreview.prototype.setupBorderSub = function() {
    ...
    makeBorderPart(0, (fbCanvas.height - this.border), this.border, this.border, voidColor, fbCanvas);
	makeBorderPart((fbCanvas.width - this.border), (fbCanvas.height - this.border), (this.border + 1), this.border, voidColor, fbCanvas);
    makeBorderPart((fbCanvas.width - this.border), 0, (this.border + 1), this.border, voidColor, fbCanvas);
  }

  StandardCushionPreview.prototype.setupBorder = function() {
    var passLeft = ((this.canvasWidth - (this.originalWidth / this.formatWidth * this.printWidth)) / 2);
    var passTop = ((this.canvasHeight - (this.originalHeight / this.formatHeight * this.printHeight)) / 2);

    makeBorderPart(0, 0, passLeft, this.canvasHeight, "#ffffff", fbCanvas, false, false, "BorderLeft");
    makeBorderPart(0, 0, this.canvasWidth, passTop, "#ffffff", fbCanvas, false, false, "BorderTop");
    makeBorderPart((this.canvasWidth - passLeft), 0, passLeft, this.canvasHeight, "#ffffff", fbCanvas, false, false, "BorderRight");
    makeBorderPart(0, (this.canvasHeight - passTop), this.canvasWidth, passTop, "#ffffff", fbCanvas, false, false, "BorderBottom");
	...
	insertSafeZone(this.material, this.formatWidth, this.formatHeight, this.canvasWidth, this.canvasHeight, passLeft, passTop);
  }
  ...
  PhoneCasePreview.prototype.setupBorder = function() {
    insertBorderPart(0, 0, this.leftBorder, this.canvasHeight, "transparent", fbCanvas, "SaveBorderLeft");
    insertBorderPart(0, 0, this.canvasWidth, this.topBorder, "transparent", fbCanvas, "SaveBorderTop");
    insertBorderPart(this.canvasWidth - this.rightBorder, 0, this.rightBorder, this.canvasHeight, "transparent", fbCanvas, "SaveBorderRight");
    insertBorderPart(0, this.canvasHeight - this.bottomBorder, this.canvasWidth, this.bottomBorder, "transparent", fbCanvas, "SaveBorderBottom");

	insertSafeZone(this.material, this.formatWidth, this.formatHeight, this.canvasWidth, this.canvasHeight, this.border, this.border);
  }

  TshirtPreview.prototype.setupBorder = function() {
    insertBorderPart(0, 0, this.leftBorder, this.canvasHeight, "#ffffff", fbCanvas, "BorderLeft");
    insertBorderPart(0, 0, this.canvasWidth, this.topBorder, "#ffffff", fbCanvas, "BorderTop");
    insertBorderPart(this.rightBorder, 0, this.leftBorder, this.canvasHeight, "#ffffff", fbCanvas, "BorderRight");
    insertBorderPart(0, this.bottomBorder, this.canvasWidth, this.canvasHeight - this.bottomBorder, "#ffffff", fbCanvas, "BorderBottom");
  ...
  CanvasPreview.prototype.drawBorders = function() {
    ...
    img.onload = function() {
      if ('BLACK' == preview.bordereffect) {
        makeColorBorder(fbCanvas.width, fbCanvas.height, preview.border, "#000000", false);
      } else if ('WHITE' == preview.bordereffect) {
        makeColorBorder(fbCanvas.width, fbCanvas.height, preview.border, "#ffffff", true);
        ...
        preview.drawCanvasSample(img);
        preview.makeMirrored(preview.printWidth, preview.printHeight, preview.border | 0);
      }
      if (preview.tiles > 1) {
        preview.buildTiles(preview.lineWidth, preview.lineHeight, preview.border);
      }
  ...
  CanvasPreview.prototype.drawCanvasSample = function(printImage) {
    ...
    ctx.drawImage(printImage, sourceX, sourceY, sourceWidth, sourceHeight, destX, destY, destWidth, destHeight);
  }
  ...
  CanvasPreview.prototype.makeStretched = function(border) {
    ...
    ctx2.drawImage(ctx.canvas, sourceX, sourceY, sourceWidth, sourceHeight, destX, destY, destWidth, destHeight);
  ...
  Preview.prototype.setupParam = function(offsetX, offsetY) {
    ...
    document.getElementById('itemConfiguratorForm:cutoutWidth').value = cutW;
    document.getElementById('itemConfiguratorForm:cutoutHeight').value = cutH;
    if (!!document.getElementById('rotateMotif')) {
      document.getElementById('itemConfiguratorForm:cutoutRotation').value = this.rotation;
    }
  }
  ...
  Preview.prototype.pictureMoving = function(topBound, leftBound) {
    ...
    bgImage.setLeft(Math.max(Math.min(left, leftBound), (rightBound) - bgImage.getWidth()));
    bgImage.setTop(Math.max(Math.min(top, topBound), (bottomBound) - bgImage.getHeight()));
  }
  ...
  CanvasPreview.prototype.buildTiles = function(canvasW, canvasH, border) {
    ...
    for (u = 1; u < this.tiles; u++) {
      if (canvasW > canvasH) {
        makeBorderPart(i, 0, space, fbCanvas.height, voidColor, fbCanvas, true);
      } else {
        makeBorderPart(0, i, fbCanvas.width, space, voidColor, fbCanvas, true);
      }
      i = i + tile + space;
    }
  }
  ...
  function makeFrameBorderPart(l, t, w, h, color, canvas, eventedOff, outline, PartName) {
	...
	var FrameMainFile = "/staticimages/configurator/" + (RetinaDisplay ? 'x2/' : '') + "frame_" + (Frametype == "Decor" ? decoFrames[('f' + preview.frame)] : premiumFrames[('f' + preview.frame)]);
		...
		if (imgFrame != null) {
			...
			imgRotate.onload = function() {
				...
				if (side == "top" || side == "bottom") {
					pol = new fabric.Polygon([{x: 0, y: 0}, {x: FrameBorder, y: FrameBorder}, {x: FrameWidth - FrameBorder, y: FrameBorder}, {x: FrameWidth, y: 0}]);
				}
				else if (side == "left" || side == "right") {
					pol = new fabric.Polygon([{x: 0, y: 0}, {x: FrameBorder, y: FrameBorder}, {x: FrameHeight - FrameBorder, y: FrameBorder}, {x: FrameHeight, y: 0}]);
				}
				...
				*/
				...
				pol.fill = new fabric.Pattern({source: RotCanvas, repeat: 'repeat'});

				// Use result as a Image
				new fabric.Image.fromURL(pol.toDataURL("image/png"), function(imgOut) {
				  ...
				  id : (PartName ? "MapImage-" + PartName + "-" + Frametype : ""), hasControls : hasControls, hasBorders : hasBorders, selectable : selectable, evented : evented
				});
				...
				if (Overlap_Gap < 1) {
					...
					imgRotateSub.onload = function() {
						...
						if (side == "top" || side == "bottom") {
							polSub = new fabric.Polygon([{x: 0, y: 0}, {x: FrameSubBorder, y: FrameSubBorder}, {x: FrameWidth - FrameSubBorder, y: FrameSubBorder}, {x: FrameWidth, y: 0}]);
						}
						else if (side == "left" || side == "right") {
							polSub = new fabric.Polygon([{x: 0, y: 0}, {x: FrameSubBorder, y: FrameSubBorder}, {x: FrameHeight - FrameSubBorder, y: FrameSubBorder}, {x: FrameHeight, y: 0}]);
						}
						...
						polSub.fill = new fabric.Pattern({source: RotCanvasSub, repeat: 'repeat'});

						// Use result as a Image
						new fabric.Image.fromURL(polSub.toDataURL("image/png"), function(imgOutSub) {
							...
							id : (PartName ? "MapImage-Sub-" + PartName + "-" + Frametype : ""), hasControls : hasControls, hasBorders : hasBorders, selectable : selectable, evented : evented
						});
					}
			...
			makeBorderPart(l, t, w, h, color, canvas, eventedOff, outline, PartName);
  ...
  function makeShadowParams(l, t, w, h, canvas, Overlap_Gap, Frametype) {
	...
	ShadowColor = (Frametype == "Decor" ? "rgba(0,0,0,0.6)" : "rgba(0,0,0,0.6)");
  ...
  function makeShadowCanvas(l, t, w, h, canvas, Overlap_Gap, Frametype) {
	...
	if (Frametype == "Premium") {
		...
		ctx.shadowColor = (preview.frame == 16 || preview.frame == 17 ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,1)");
		...
		new fabric.Image.fromURL(canvasShadow.toDataURL("image/png"), function(imgShadow) {
			...
			id : ShadowId + "Canvas", hasControls : false, hasBorders : false, selectable : false, evented : false
		});
	}
  }
  ...
  function makeBorderPart(l, t, w, h, color, canvas, eventedOff, outline, PartName) {
  ...
  function insertSafeZone(material, formatWidth, formatHeight, canvasWidth, canvasHeight, border_H, border_V) {
	...
	if (leftBorder > numberZero) {
		insertBorderPart(0, 0, leftBorder, canvasHeight, "transparent", fbCanvas, "SaveBorderLeft");
	}
	if (rightBorder > numberZero) {
		insertBorderPart(canvasWidth - rightBorder, 0, rightBorder, canvasHeight, "transparent", fbCanvas, "SaveBorderRight");
	}
	if (topBorder > numberZero) {
		insertBorderPart(0, 0, canvasWidth, topBorder, "transparent", fbCanvas, "SaveBorderTop");
	}
	if (bottomBorder > numberZero) {
		insertBorderPart(0, canvasHeight - bottomBorder, canvasWidth, bottomBorder, "transparent", fbCanvas, "SaveBorderBottom");
	}
  }
...
//		makeBorderPartSub(canvaW - border, 0, border, canvaH, subBorderColor, fbCanvas, subBorderIndex, "HideColorRectFlat"); // RIGHT
  ...
  function makeFrameBorderOver(canvaW, canvaH, border, canvas, color, cutout) {
	...
	makeBorderPart(0, 0, canvaW, border, "transparent", canvas, false, false, "BorderTop"); // TOP
	makeBorderPart(0, canvaH - border, canvaW, border, "transparent", canvas, false, false, "BorderBottom"); // BOTTOM
	makeBorderPart(0, 0, border, canvaH, "transparent", canvas, false, false, "BorderLeft"); // LEFT
	makeBorderPart(canvaW - border, 0, border, canvaH, "transparent", canvas, false, false, "BorderRight"); // RIGHT

	makeFrameBorderPart(0, 0, canvaW, border, color, canvas, false, false, "BorderTop"); // TOP
	makeFrameBorderPart(0, canvaH - border, canvaW, border, color, canvas, false, false, "BorderBottom"); // BOTTOM
	makeFrameBorderPart(0, 0, border, canvaH, color, canvas, false, false, "BorderLeft"); // LEFT
	makeFrameBorderPart(canvaW - border, 0, border, canvaH, color, canvas, false, false, "BorderRight"); // RIGHT

	makeShadowCanvas(0, 0, preview.border, preview.border, canvas, 0, "Premium");
  }

  function makeColorBorder(canvaW, canvaH, border, color, cutout) {

	makeBorderPart(0, 0, canvaW, border, color, fbCanvas, false, false, "BorderTop"); // TOP
	makeBorderPart(0, canvaH - border, canvaW, border, color, fbCanvas, false, false, "BorderBottom"); // BOTTOM
	makeBorderPart(0, 0, border, canvaH, color, fbCanvas, false, false, "BorderLeft"); // LEFT
	makeBorderPart(canvaW - border, 0, border, canvaH, color, fbCanvas, false, false, "BorderRight"); // RIGHT

	if (cutout) {
		//l, t, w, h, color, canvas, eventedOff, outline
		makeBorderPart(border + 1,	1, canvaW - (border * 2) - 2, border - 1, color, fbCanvas, false, true); // TOP
		makeBorderPart(border + 1, canvaH - border, canvaW - (border * 2) - 2, border - 1, color, fbCanvas, false, true); // BOTTOM
		makeBorderPart(1, border + 1, border - 1, canvaH - (border * 2) - 2, color, fbCanvas, false, true); // LEFT
		makeBorderPart(canvaW - border, border + 1, border - 1, canvaH - (border * 2) - 2, color, fbCanvas, false, true); // RIGHT
	}
  }
